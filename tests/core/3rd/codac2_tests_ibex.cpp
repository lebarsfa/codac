...
TEST_CASE("IBEX")
{
  ...
  // [Vector] Codac -> IBEX
  {
    CHECK(codac2::to_ibex(codac2::Vector::zeros(2)) == ibex::Vector(2));
    CHECK(codac2::to_ibex(codac2::Vector({1,2,3})) == ibex::Vector({1,2,3}));
  }

  // [Vector] IBEX -> Codac
  {
    CHECK(codac2::to_codac(ibex::Vector::zeros(2)) == codac2::Vector::zeros(2));
  ...
  // [Matrix] Codac -> IBEX
  {
    CHECK(codac2::to_ibex(codac2::Matrix::zeros(2,3)) == ibex::Matrix(2,3));
  ...
  // [Matrix] IBEX -> Codac
  {
    CHECK(codac2::to_codac(ibex::Matrix(2,3)) == codac2::Matrix::zeros(2,3));
